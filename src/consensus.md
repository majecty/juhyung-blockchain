...
처음엔 PoW로 작성하고 뒤에 PoS 컨센서스 엔진을 추가했다.
...
블록 전파은 다음과 같이 동작한다. 블록을 생성하는 노드가
규칙에 맞게 블록을 생성하면, 해당 노드는 새로 생성한 블록을 피어들에게 뿌린다.
새로운 블록을 받은 피어는 해당 블록을 검증한 뒤에 문제가 없으면 다시
자신의 피어들에게 뿌린다.

큰 하는 일만 봐서는 컨센서스 알고리즘의 교체가 블록 전파에 미치는 영향은 없을 것으로 보인다.
...
PoS는 그렇지 않았다. PoS는 블록의 헤더에 블록을 생성하는 위원회의 서명을 담는다.
PoW는 블록체인 스테이트 없이 헤더의 해시 만으로 검증이 가능했지만, PoS는 그렇지 않다.
위원회의 목록을 스테이트에서 알아와야하기 때문이다.
...
결론: ???
일하다 이런 문제를 만나면 곤혹스럽다.
하기로 한 건 내일까지고, 계획했던 건 내일까지 되겠는데 다른 게 터지는 걸 어떡하나.
상황을 빠르게 매니저와 공유하고, 일정을 다시 조정하겠지만,
뒤늦게 발견해 일정이 꼬이는 상황이 스트레스다.

이전엔 프로그램의 동작방식만 이해하고 있으면, 변경사항의 영향력을 잘 예측할 수 있을거라
생각했다. 요즘은 좀 다르다. 코드에 담긴 의도와 가정들을 잘 이해해야 한다고 생각한다.
만약 헤더를 따로 받지 않고, 매 번 한 블록씩 받아서 검증했다면 블록 싱크와 컨센서스는
서로 독립적이었을 것이다.
블록 싱크 과정을 최적화하기 위해서 PoW 컨센서스의 특징을 사용했을 때부터 두 컴포넌트의
인연이 시작되었다. 이 인연은 디테일을 살펴보지 않으면 잘 드러나지 않았다.

처음부터 문제를 예측할 수 있으며련 모든 디테일을 암기하고 있는 사람이 있으면 되겠다.
하지만 프로그램은 갈수록 커지고 기억은 시간이 지날수록 휘발된다.
한 사람이 모든 디테일을 기억할 수 없다.
한 사람 한 사람이 프로그램의 분야를 나누어서 디테일을 기억하는 게 좋다.
디테일을 다 기억하지 못하더라도, 내가 아는 컴포넌트가 다른 컴포넌트와
어떤 관계를 가지고 있는지 정도 아는 게 좋다.
A가 블록 싱크쪽 코드를 관리하고, B가 컨센서스 알고리즘을 관리한다면
주기적으로 서로 무슨 일을 하고 있는지 공유해야 한다.

전 회사에서는 각자가 하는 일을 자주 공유했었다. 종종 일을 들어갈 때
어떤 일을 시작하고 어떤 영향을 받을지 간단하게 이야기한다.
B가 PoS 컨센서스를 구현한다고 공유했을 때 A가 블록싱크의 헤더 최적화를 기억했다면
미리 발생할 문제를 체크할 수 있다.

나는 특정 사람이 특정 코드에 소유권을 갖는 거에 반대한다.
